...
module Partial_ordering = struct
  ...
  let compare t1 t2 =
    let open Std.PartialOrdering in
    let t11, t12 = t1 in
    let t21, t22 = t2 in
  ...
  let of_str s = Stdlib.Scanf.sscanf s "%d %d" (fun x1 x2 -> (x1, x2))
...
module Naive_radix = struct
  ...
  let rec map f key tree =
    ...
    | K_int x :: key', Int_layer xs ->
        let tree' = List.Assoc.find_exn ~equal:Int.equal xs x in
        map f key' tree'
    | K_string x :: key', String_layer xs ->
        let tree' = List.Assoc.find_exn ~equal:String.equal xs x in
        map f key' tree'
    | _ -> failwith "case mismatch"
  ...
  let rec subst key tree t2 =
    ...
    | K_string x :: key', String_layer xs ->
        String_layer
          (List.map
             ~f:(fun (ki, ti) ->
               let ti' = if String.(ki = x) then subst key' ti t2 else ti in
               (ki, ti'))
             xs)
    | _ -> failwith "case mismatch"
