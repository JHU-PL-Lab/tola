...
dune exec src/bin/canary_main.exe -- action @all           # THE batch: every registry project under the default config — Heavy (z3/llvm) THIN (bypasses the source-built Dev chains), Light FULL; --thin forces thin everywhere; --audit-lib = full + Materialize_source (the shadowed source-built placements materialize — blame-driven, never in the batch); single-project runs always full
