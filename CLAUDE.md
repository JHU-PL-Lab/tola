...
| `src/canary/base/canary_artifact.ml`                | Artifact IDENTITY: `artifact_info` (a SUM since 2026-08-24 — `A_binding of lang * mechanism`, `A_app of app_wiring`, …; was a `{ kind; ext }` record whose pairing was convention-only), `kind_of` projecting the coarse `artifact_kind`, `mechanism_of`/`wiring_of` for the refinements (`artifact_ext`/`ext_of` retired the same day — every consumer wanted something narrower), the `a_*` smart constructors, `artifact_axes`, `project_spec`, `placement`, `assignment` |
