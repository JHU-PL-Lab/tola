...
dune exec src/bin/canary_main.exe -- action z3               # GENERIC path (A5 + C2 3-way + C3 regression ref): 7 scenarios = 4 all-Fetched source worlds (stable 4.15.2 / official latest / arbipher fork / pre-10549) + 3 dev build chains; parser_context xfail fires in every world (scenario-invariant). --thin = stable chain only. --refs latest,pre-10549 = THE regression pair (4 scenarios; the pre-10549 Install_lib xfails "OCAML INSTALL MISSING", latest passes — the #10549 bugfix-commit regression test). --installed = the installed-consumer policy (consumer_lib=Installed: z3's dev-chain probe_binding OCaml reads the STAGED prefix <build>/../install instead of the build tree; scenario set unchanged — a REALIZATION choice, not an enumeration axis). With --refs latest,pre-10549 --installed: latest probes the staged package (PASS), pre-10549 xfails at BOTH install and probe ("STAGED PACKAGE MISSING" — the #10549 bug visible ON the consumer)
