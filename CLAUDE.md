...
| `doc/canary/design/enumeration/store_switching.md` | The opam switch as ONE instance: what one-version-per-switch costs, the per-version-switch measurement (`ocaml-system` = ~5 s), and the two open questions (which switch model; what a collateral rebuild is FOR) |
