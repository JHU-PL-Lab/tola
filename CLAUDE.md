...
| `doc/canary/design/dynamic_enumeration.md`     | Dynamic-enumeration SSOT (2026-08-04): an artifact can be a built result — a resource generates (`built_from`) or contains (bundle) other artifacts, so `provision` is a per-EDGE provider, not a flat label. Companion to enumeration_graph.md; gates convergence A3b/A4 (status §A) |
