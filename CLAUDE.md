...
| `doc/canary/design/enumeration/store_switching.md` | What a version pin COSTS: one version per package per switch, the pin as an exclusive LOCK (run order groups by it, §6), design B measured, and the two open questions (which switch model; what a collateral rebuild is for) |
