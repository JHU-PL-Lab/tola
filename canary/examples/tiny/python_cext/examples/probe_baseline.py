"""Baseline probe for the CPython C-extension binding (static FFI).

Exercises every wrapper in ``tiny_cext`` and checks the results
against the values an unbroken native side should produce. Exits
non-zero on the first mismatch.
"""

import sys
import tiny_cext as tiny
...
def main() -> None:
    ...
    check("tiny.sum(2, 3)", 47, tiny.sum(2, 3))
    check("tiny.diff(5, 2)", 3, tiny.diff(5, 2))
    check("tiny.diff(2, 5)", -3, tiny.diff(2, 5))
