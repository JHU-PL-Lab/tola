...
SCENARIOS = {
    ...
    "header_arity_bump": {
        "description": "tiny.h declares tiny_sum with an extra (int c) parameter; tiny.c matches the new signature so the lib still builds. The cstub (tiny_stubs.c) calls tiny_sum(a, b) — only 2 args, expecting the baseline declaration. When canary rebuilds the OCaml binding against the perturbed header, the cstub compile fails with `error: too few arguments to function 'tiny_sum'`. c6 cmp_type catches the static mismatch by diffing the header's signature (3 args) against the binding stub's expected signature (2 args).",
        "violates": ["Type"],
        "perturbs": ["c/include/tiny.h", "c/src/tiny.c"],
        "apply":  _c_patch_apply("header_arity_bump"),
        "revert": _c_patch_revert("header_arity_bump"),
        "expected": {
            "ocaml_build":             "fail",
            "ocaml_probe":             "skip",
