...
let do_find_macro =
  yc_macro (ystr "do_find") ~args:[ "MODULE_NAME" ]
    [
      ...
      yc_set (ycvar "CMAKE_MODULE_PATH") [ ystr_raw "${ORIGINAL_MODULE_PATH}" ];
    ]
                ...
                yifthen
                  ...
                  (yc_message ~mode:Mm_send_error
                     [ "unexpected: ${VERSION_VAR} does not begin with a \
                        decimal digit" ]);
                yifthen
                  (ystrequal (ystr_raw "${${VERSION_VAR}}") (ystr ""))
                  (yc_message ~mode:Mm_send_error
                     [ "unexpected: ${VERSION_VAR} is empty" ]);
                yifthen
                  (yversion_equal (ystr_raw "${${VERSION_VAR}}") (ystr "0"))
                  (yc_message ~mode:Mm_send_error
...
let glob_modules =
  yc_file_glob (ycvar "FIND_MODULES")
    [ ystr_raw "${CMAKE_CURRENT_SOURCE_DIR}/../../../Modules/Find*.cmake" ]
    ...
    (ycmd_of_list
       [
         yc_string_regex_replace
           ".*/Find(.*)\\.cmake$" (ystr "\\\\1")
           (ycvar "MODULE_NAME") [ ystr_raw "${FIND_MODULE}" ];
         yc_apply (ystr "do_find") [ ystr_raw "${MODULE_NAME}" ];
       ])
...
let vtest_version_string =
  yc_foreach (ycvar "VTEST")
    ...
    (yc_apply (ystr "check_version_string")
       [ ystr_raw "${VTEST}"; ystr_raw "${VTEST}_VERSION_STRING" ])
  ...
  yc_foreach (ycvar "VTEST")
    ~items:(List.map ystr
      [ "Armadillo"; "BISON"; "Boost"; "BZip2"; "BZIP2"; "Doxygen"; "DOXYGEN";
        ...
        "wxWidgets"; "ZLIB" ])
    (yc_apply (ystr "check_version_string")
       [ ystr_raw "${VTEST}"; ystr_raw "${VTEST}_VERSION" ])
...
let cmd =
  ycmd_of_list
    [
      ...
      yc_set (ycvar "ORIGINAL_MODULE_PATH") [ ystr_raw "${CMAKE_MODULE_PATH}" ];
      do_find_macro;
