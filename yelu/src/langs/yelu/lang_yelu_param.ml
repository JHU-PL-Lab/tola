(** Parametric AST shapes for yelu — full coverage of all groups.
    ...
    directly from [Lang_yelu] / [Lang_cmake]. We only abstract over
    ...
    Status: design module. Once instantiated with cmake substrate
    (in [Lang_yelu_lower]) it is the structural mirror of yelu's
    AST groups. Future packs would create their own concrete instances.
*)
...
module Make_target_op (T : LANG_TYPES) = struct
  ...
  type target_sources_item =
    | Ytsi_plain of items_with_kind
    | Ytsi_file_set of file_set
  ...
  type t =
    ...
    | Ytgt_add_library of {
        name : T.arg;
        type_ : Lang_yelu.library_type option;
        exclude_from_all : bool;
        sources : T.arg list;
      }
    ...
    | Ytgt_sources of { target : T.arg; items : items_with_kind list }
    | Ytgt_sources_fs of { target : T.arg; items : target_sources_item list }
...
module Make_install_op (T : LANG_TYPES) = struct
  type t =
    ...
    | Yinstall_write_basic_package_version_file of {
        ...
        compatibility : Lang_yelu.compatibility;
...
module Make_cmake_op (T : LANG_TYPES) = struct
  type t =
    ...
    | Ycmake_project of {
        ...
        languages : Lang_yelu.supported_lang list;
      }
...
(* ============================================================
   ...
   This is the parametric mirror of [Lang_yelu.yelu_exp]. Constructor
   names match yelu's so that lowering is mechanical (constructor
   pattern → same-name constructor in Lang_yelu).
   ============================================================ *)
...
module Make_exp (T : LANG_TYPES) = struct
  ...
  type t =
    | Ye_string of String_op.t
    | Ye_list of List_op.t
    | Ye_file of File_op.t
    | Ye_target of Target_op.t
    | Ye_dir of Dir_op.t
    | Ye_state of State_op.t
    | Ye_find of Find_op.t
    | Ye_install of Install_op.t
    | Ye_test of Test_op.t
    | Ye_try of Try_op.t
    | Ye_cmake of Cmake_op.t
    (* core *)
    | Ylet of { name : string; value : T.arg }
    | Yif of { cond : Cond.t; then_ : t; else_ : t option }
    | Yexp_list of t list
    (* scripting *)
    | Yc_include of { file : T.arg; optional : bool }
    | Yc_function of { name : T.arg; args : string list; body : t list }
    | Yc_macro of { name : T.arg; args : string list; body : t list }
    ...
    (* control flow *)
    | Yc_foreach of { loop_var : T.var; items : T.arg list; commands : t }
    | Yc_foreach_range of {
        loop_var : T.var;
        start : int option;
        stop : int;
    ...
    | Yc_while of { cond : Cond.t; commands : t }
    | Yc_break
    | Yc_continue
