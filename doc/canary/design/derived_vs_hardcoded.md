...
| Per-project hand-written | `canary_project_<name>.ml` — `project_spec` + `api_source` + optional expectation predicate                                                                | This one project's shape (watchlists, shell strings, install patterns).                             | **Yes — this only**     |
