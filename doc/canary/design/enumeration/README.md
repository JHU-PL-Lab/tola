...
| --- | --- | --- |
| 0. vocabulary | [`stage0_naming.md`](stage0_naming.md) | **done** — the four senses, the naming scheme, the fault tags |
| 1. declaration | [`stage1_project_spec.md`](stage1_project_spec.md) | **done** — absorbed `repo_model.md` + `versioning.md` |
| 2. enumeration | [`stage2_filters.md`](stage2_filters.md) | **done** — the five constraints have one owner |
| 3. identity + order | [`stage3_identity.md`](stage3_identity.md) | **done** — identity/dedup, the general exclusive-resource principle, run order |
| **4. realization** | **—** | **the only gap.** Material in `algorithm_explainer.md` §6, §11 |
| 5. reporting | [`stage5_matrix.md`](stage5_matrix.md) | **done** — already single-owner |

Everything else in this directory is either the walkthrough
(`algorithm_explainer.md`) or a **proposal**: `multi_lib.md` (stage 0/1 —
`Lib` carries no name) and `run_model_revisit.md` (findings across stages
2, 3 and 5). Docs that turned out not to be enumeration stages left:
`staged_parity.md` → `../staged_parity.md` (a checking principle),
`store_switching.md` → `../../project/opam_exclusive_store_issue.md`
(one package manager's problem).
...
| **2. enumeration**      | the product, then the constraints that prune it                                                                                                    | `action/canary_enumerate.ml` (`run_config`, `enumerate_points`, `assignment_of_point`, then `assignment_ok`, `ax_follows`, `binding_couples`, `source_ref_ok`, `shadow_filter`, `ref_filter`)            | [`algorithm_explainer.md`](algorithm_explainer.md) §4–5, **[`stage2_filters.md`](stage2_filters.md)** for the constraints                                                                                                                            | `enumerate.config_levels`, `enumerate.subset_intersects_universe`, `enumerate.thin_is_version_subset`, `enumerate.refs_subset`, `enumerate.shadow_policy_drops_same_cell_built`, `enumerate.point_to_assignment_fold`, `enumerate.two_projections_and_filter`, `enumerate.version_axis`, `enumerate.built_from_of_assignment`, `enumerate.mismatch_direction`, `enumerate.deploy_mismatch`, `shadow.policy_ladder` |
...
| **5. reporting**        | what a row is and what names it                                                                                                                    | `main/canary_matrix.ml`, `backend/canary_status.ml`, `backend/canary_html.ml`                                                                                                                            | [`stage5_matrix.md`](stage5_matrix.md)                                                                                                                                                                                                               | `matrix.row_index`, `matrix.row_order`, `matrix.cell_stage_progression`, `matrix.setting_block_identifies_world`, `matrix.marks_from_log`                                                                                                                                                                                                                                                                          |
