...
| stage | standalone doc | state |
| --- | --- | --- |
| 1. declaration | [`stage1_project_spec.md`](stage1_project_spec.md) | **done** — readable on its own; the docs it drew from keep only what is not stage 1 |
| 2. enumeration | [`filters.md`](filters.md) | **done** in substance — the constraints have one owner |
| 3. identity + order | — | material still in `store_switching.md` §5g and `versioning.md` §2 |
| 4. realization | — | material still in `algorithm_explainer.md` §6, §11 |
| 5. reporting | [`matrix.md`](matrix.md) | already single-owner |

`algorithm_explainer.md` is the predecessor of this README — the
end-to-end walkthrough with z3 carried through as a running example. It
stays for human readability; where a stage doc exists, that doc is
authoritative and the walkthrough points at it.
...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **0. vocabulary**       | the types every stage reuses: `artifact_kind`, `provision`, `channel`, `version`, `build_id`, `artifact_id`, `placement`, `assignment`, `dep_mode` | `base/canary_basic.ml`, `base/canary_store.ml`, `base/canary_artifact.ml`                                                                                                                                | [`../ssot.md`](../ssot.md) (IDs), [`scenario.md`](scenario.md) (the four senses of "scenario")                                                                                                                                         | `vocab.binding_source_off_tree`, `surface.split_keeps_checks_drops_provenance`, `scenario.lower_expectation_agnostic_c1`                                                                                                                                                                                                                                                                                           |
| **1. declaration**      | a project states which artifacts exist, at which provisions, at which versions, and who provides each                                              | `action/canary_project_spec.ml` (`artifact_row`, `project_spec_of_rows`), `base/canary_artifact.ml` (`axes = {ax_universe; ax_runtime; ax_follows; ax_pins}`), `tool/canary_store_config.ml` (providers) | **[`stage1_project_spec.md`](stage1_project_spec.md)** — THE stage-1 doc, standalone. Background: [`repo_model.md`](repo_model.md) repo lifecycle, [`store_switching.md`](store_switching.md) what a pin costs, [`versioning.md`](versioning.md) the identity model, [`multi_lib.md`](multi_lib.md) what cannot be declared | `enumerate.project_spec_sqlite_shape`, `enumerate.per_artifact_provisions`, `enumerate.per_artifact_versions`, `enumerate.per_provision_versions`, `repo_model.axes_pins`, `repo_model.contents_invariant`, `spec.vendored_prebuilt_pair`, `spec.pm_dep_gate_groups`, `sqlite.provider_rows`, `z3.provider_rows`                                                                                                   |
| **2. enumeration**      | the product, then the constraints that prune it                                                                                                    | `action/canary_enumerate.ml` (`run_config`, `enumerate_points`, `assignment_of_point`, then `assignment_ok`, `ax_follows`, `binding_couples`, `source_ref_ok`, `shadow_filter`, `ref_filter`)            | [`algorithm_explainer.md`](algorithm_explainer.md) §4–5, **[`filters.md`](filters.md)** for the constraints                                                                                                                            | `enumerate.config_levels`, `enumerate.subset_intersects_universe`, `enumerate.thin_is_version_subset`, `enumerate.refs_subset`, `enumerate.shadow_policy_drops_same_cell_built`, `enumerate.point_to_assignment_fold`, `enumerate.two_projections_and_filter`, `enumerate.version_axis`, `enumerate.built_from_of_assignment`, `enumerate.mismatch_direction`, `enumerate.deploy_mismatch`, `shadow.policy_ladder` |
| **3. identity + order** | which assignments are the SAME scenario, and in what order they run                                                                                | `project/canary_project_run.ml` (`scenarios_of`, `scenario_dir_of`, `store_state_key`, `scenarios_in_run_order`)                                                                                         | [`store_switching.md`](store_switching.md) §5g (the pin as a lock), [`versioning.md`](versioning.md) §2 (ambient vs identity-bearing)                                                                                                  | `run_order.groups_by_store_state`, `matrix.registry_shape`                                                                                                                                                                                                                                                                                                                                                         |
| **4. realization**      | assignment → commands → steps; and the parallel node-graph view                                                                                    | `project/*` (`pr_runner_spec = realize ∘ dispatch`), `action/canary_step_builder.ml` (`derive_steps`), `action/canary_action.ml` (`node_of_assignment`, `close_deps`, `execution_plan`)                  | [`algorithm_explainer.md`](algorithm_explainer.md) §6, §11; [`../action_playbook.md`](../action_playbook.md) to add an action                                                                                                          | `action.node_of_assignment_chain`, `action.close_deps_deploy_mismatch`, `action.execution_plan_topo_and_edges`, `arrow.providing_action_total_and_consistent`, `enumerate.dispatch_coordinate_reads`, `z3.dispatch_reads_source_placement`, `derive.fetch_lib_matches_helper`, `probe_invariant.consumes_eq_artifacts`                                                                                             |
| **5. reporting**        | what a row is and what names it                                                                                                                    | `main/canary_matrix.ml`, `backend/canary_status.ml`, `backend/canary_html.ml`                                                                                                                            | [`matrix.md`](matrix.md)                                                                                                                                                                                                               | `matrix.row_index`, `matrix.row_order`, `matrix.cell_stage_progression`, `matrix.setting_block_identifies_world`, `matrix.marks_from_log`                                                                                                                                                                                                                                                                          |
...
| [`run_model_revisit.md`](run_model_revisit.md) | findings from the 2026-08-20 reruns that land in stages 2, 3 and 5 — a proposal list, not a description              |
...
- The **cache** — `../artifact_cache.md` (proposal) and
  `algorithm_explainer.md` §9 (what exists).
...
Every pin name cited above is checked by `docs.cited_pins_exist`
(`main/canary_projects_test.ml`): if a doc in this directory names a pin
that no longer exists, `make canary-test` fails. That is deliberate — a
design doc citing a deleted pin is exactly how these docs went stale
before, and prose cannot notice it.

The converse is not automated: a pin can exist while the doc around it
describes something the code stopped doing. When you land a change to a
stage, re-read that stage's row.
