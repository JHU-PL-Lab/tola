...
Step-level renderer (`mermaid_full` in `canary_action.ml`). Every step is an
...
Each view is generated by `mermaid_view` in `canary_action.ml`. All focused
...
3. Loads the step's output files in the detail panel (probe.log,
   summary.json, etc.) via relative fetch.
