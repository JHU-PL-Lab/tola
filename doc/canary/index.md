...
| [source_provisioning.md](design/source_provisioning.md) | **Getting the source** — a cross-pass topic, like `staged_parity`: the worktree-per-ref shape, why not submodules, why partial clone and not shallow, why an unread fetch is not realized, prepare-once/ensure-per-world, and what a CI job actually costs. Open work is in trackers (its §7) |
