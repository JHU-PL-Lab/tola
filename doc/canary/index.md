...
| [source_provisioning.md](design/source_provisioning.md) | **How canary obtains source, and what it costs** — a cross-pass topic, like `staged_parity`. Why project sources are not submodules; why partial clone and not shallow (history, not refs); why an unread fetch is not realized; prepare-once/ensure-per-world; and what a CI job actually pays for. Open items are in trackers, listed in its §7 |
