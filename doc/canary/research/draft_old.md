...
- **§2 Tiny: a running example.** The minimal touchstone — one
  ...
  detail in [`tiny.md`](surface_draft/tiny.md).
...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------- | ------------------------------- | ---------------------------------------------------------------------------- |
| **Type**             | [e3 `type_wrong`](surface_draft/tiny.md#e3-type_wrong--type-contract-manifests-as-behavior) (body, Ag.3 fires); canary-added `header_arity_bump` (static Ag.6)       | n3 + bo1 (inspect_tiny_typed.py) | **Ag.6** `cmp_type`             | ✓ static comparator over `Typed_header` + `Typed_binding_stub` (Phase 15.5b) |
| **Symbol**           | [e1 `symbol_missing`](surface_draft/tiny.md#e1-symbol_missing--symbol-contract), [e8 `symbol_orphan`](surface_draft/tiny.md#e8-symbol_orphan--symbol-contract-binding-side-orphan) | n4 + bo7                         | **Ag.1** `cmp_symbol`           | ✓ `check_c_compat` — comparator over `C_stub` + `Native_lib`                 |
...
| **ABI**              | [e2 `abi_soname_bump`](surface_draft/tiny.md#e2-abi_soname_bump--abi-contract)                                                                                       | n4 + bo6 (or bo7)                | **Ag.4** `cmp_abi`              | ✓ comparator over `Native_lib` + `Abi_surface` (Phase 14e)                   |
| **API-sound-repack** | [e5 `api_repack`](surface_draft/tiny.md#e5-api_repack--intra-binding-repacking-ocaml-only) (OCaml), e10 `api_repack_python` (Python)                                 | binding-side test                | **Ag.7** `api_sound_repack`     | ✓ probe runner + `Expect_failure` (binding-side refutation; Phase 15.6)      |
| **API-completeness** | [e6 `api_complete`](surface_draft/tiny.md#e6-api_complete--api-completeness-ocaml-only) (OCaml), e11 `api_complete_python` (Python parallel)                         | bo4 or bpc2 / bpe2               | **Ag.2** `cmp_api_completeness` | ✓ watchlist inside `Expect_compat_failure { Ocaml_mli, Python_attrs }`       |
...
| **Behavior**         | [e7 `behavior_silent`](surface_draft/tiny.md#e7-behavior_silent--behavior-contract) + every other scenario's probe                                                   | probe + reference                | **Ag.3** `cmp_behavior`         | ✓ probe runner + `Expect_success` / `Expect_failure`                         |
```
...
— see [`tiny.md`](surface_draft/tiny.md)) that exercises repacking under a
