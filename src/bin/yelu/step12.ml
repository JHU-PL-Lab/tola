  ...
  ycmd_of_list
    [
      ...
      yc_project
        ...
        "Tutorial";
      yc_set
        (ycvar "CMAKE_ARCHIVE_OUTPUT_DIRECTORY")
        [ yraw "${PROJECT_BINARY_DIR}" ];
      yc_set
        (ycvar "CMAKE_LIBRARY_OUTPUT_DIRECTORY")
        [ yraw "${PROJECT_BINARY_DIR}" ];
      yc_set
        (ycvar "CMAKE_RUNTIME_OUTPUT_DIRECTORY")
        [ yraw "${PROJECT_BINARY_DIR}" ];
      yc_option ~value:(ybool true) ~msg:"Build using shared libraries"
        (ycvar "BUILD_SHARED_LIBS");
      yc_set (ycvar "CMAKE_DEBUG_POSTFIX") [ yraw "d" ];
      yc_add_library ~type_:Lib_interface (yvar "flags");
      yc_target_compile_features
        (yvar "flags")
      ...
      yc_configure_file
        ~input:(ybare "TutorialConfig.h.in")
        (ybare "TutorialConfig.h");
      ...
      yc_function (ycvar "do_test")
        [ "target"; "arg"; "result" ]
        [
          yc_add_test (ybare "Comp${arg}") (ybare "${target}")
            [ ybare "${arg}" ];
          yc_set_tests_properties
            [ ybare "Comp${arg}" ]
      ...
      yc_apply (ycvar "do_test")
        [ yvar "tut"; ybare "5"; yraw "5 is 2.236" ];
      yc_apply (ycvar "do_test")
        [ yvar "tut"; ybare "7"; yraw "7 is 2.645" ];
      yc_apply (ycvar "do_test")
        [ yvar "tut"; ybare "25"; yraw "25 is 5" ];
      yc_apply (ycvar "do_test")
      ...
      yc_set
        (ycvar "CPACK_RESOURCE_FILE_LICENSE")
        [ yraw "${CMAKE_CURRENT_SOURCE_DIR}/License.txt" ];
      yc_set
        (ycvar "CPACK_PACKAGE_VERSION_MAJOR")
        [ yraw "${Tutorial_VERSION_MAJOR}" ];
      yc_set
        (ycvar "CPACK_PACKAGE_VERSION_MINOR")
