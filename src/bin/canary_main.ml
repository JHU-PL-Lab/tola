...
let print_construction ~(name : string)
  ...
  let g =
    CA.make_action_graph
      ...
      ~versions ~name ~source:Canary_store.store ()
  in
  Fmt.pr "@.graph construction: %s (source versions: %s)@." name
...
let construct_cmd =
  ...
  let run proj () =
    ...
    match proj with
    | Some "sqlite" -> print_construction ~name:"sqlite" ~versions:vs
    | Some "tiny-full" -> print_construction ~name:"tiny-full" ~versions:vs
    | _ ->
        Fmt.epr "usage: canary construct <tiny-full|sqlite>@.";
        Stdlib.exit 2
  in
  Cmd.v
    (Cmd.info "construct"
       ~doc:
         "Show the forward graph construction: nodes generated by build/fetch \
          action-edges (incl. deploy mismatch). No run.")
    Term.(const run $ project $ const ())
