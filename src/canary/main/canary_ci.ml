...
(* The override wraps the RENDERING, not just the job construction, and
   ...
   scope has to go. *)
let render_minimal
    (projects : (string * Canary_project_run.project_run) list) : string =
  with_ci_machine_roots (fun () ->
      (* 5.4, not the backend's 5.2 default: the canary switch is OCaml
         5.4.1 and every local measurement comes from there, so a CI run
         on 5.2 would be resolving a different package universe than the
         one the verdicts were earned in. Matching it is the cheapest way
         to keep a CI disagreement meaningful. *)
      Canary_gh.render_workflow ~runner_os:"ubuntu-latest"
        ~ocaml_version:"5.4" ~triggers:minimal_triggers
        ~workflow_name:"Canary — minimal (ubuntu)" (minimal_jobs projects))
