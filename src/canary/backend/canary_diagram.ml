...
(* Reverse of the (Probe_lib | Probe_binding _ | Probe_app _) → kind
   ...
   probe action variant. Used where diagram code has an artifact_kind
   in hand and needs to reconstruct the action tag (e.g. for
   string_of_action). Probe_app is always emitted with OCaml as a
...
*)
let probe_action_of_kind = function
  | Lib -> Some Probe_lib
...
(* ── Diagram rendering ──
   ...
   The schema renderer (mermaid_of_action_graph_schema) lives in canary.ml.
...
(* summary_actions : list of (parent action, tag_suffix) pairs.

   - One pair per summary follow-up step that appears in the run.
   - Same action can appear with multiple suffixes (e.g.
   ...
   that map to a given action kind, like "probe_lib [3][4][5]" — useful
...
let mermaid_of_action_graph_schema ?status ?(has_scan = false) ?(chain_scan = false)
    ?(summary_actions : (action * string) list = [])
    ?(step_ids : (string, int) Hashtbl.t option)
    ?(steps_by_action_tag : (string, string list) Hashtbl.t option)
  ...
  let has_configure = List.exists actions ~f:(fun r ->
      match r with Configure -> true | _ -> false)
  in
  let fetch_kinds =
    List.filter_map actions ~f:(fun r -> match r with Fetch k -> Some k | _ -> None)
  in
  let build_actions =
    List.filter actions ~f:(fun r ->
        match r with
  ...
  in
  let publish_kinds =
    List.filter_map actions ~f:(fun r ->
  ...
  let probe_kinds =
    List.filter_map actions ~f:(fun r ->
        ...
        | _ -> None)
  in
  (* Artifact pool kinds: derived from actions — includes Binding lang variants *)
  ...
  (* Summary nodes are uniquely identified by (parent action, tag suffix).
     suffix is e.g. "_inspect" or "_stub_inspect". *)
  let summary_nid action suffix =
    [%string "A_%{string_of_action action}%{suffix}"]
  in
  let summary_label action suffix =
    (* e.g. fetch_binding_ocaml_stub_inspect *)
    string_of_action action ^ suffix
  in
  let source_upstream =
    if chain_scan && has_scan then scan_nid else node_id_of_kind Source
  in
  (* Action labels: when [step_ids] + [steps_by_action_tag] are supplied,
  ...
  let have_step_info =
    Option.is_some step_ids && Option.is_some steps_by_action_tag
  in
  let action_counter = ref 0 in
  let next_id () = Int.incr action_counter; !action_counter in
  let label_with_ids name action_tag =
    let ids =
      match steps_by_action_tag, step_ids with
      | Some by_tag, Some id_tbl ->
          (match Hashtbl.find by_tag action_tag with
           | Some tags ->
  ...
  (* Direct tag lookup — for follow-up steps (scan_source, *_inspect)
     that have a 1:1 step mapping but were excluded from the action-tag
  ...
  let is_inlined_inspect action =
    ...
    | _ -> false
  in
  (* Raw int IDs from steps_by_action_tag for a given canonical action name. *)
  ...
  in
  (* Raw int IDs for all summary entries of a action — used when inlining. *)
  let summary_int_ids action =
    List.concat_map summary_actions ~f:(fun (saction, suffix) ->
        if not (Poly.equal saction action) then []
        else
          let canonical = string_of_action action ^ suffix in
  ...
  let action_label_with_inspect name action =
    let all_ids =
      (action_int_ids name @ summary_int_ids action)
      |> List.sort ~compare:Int.compare
  ...
  let inline_inspect_ids action =
    summary_int_ids action
    |> List.sort ~compare:Int.compare
    |> List.map ~f:(fun n -> [%string "[%{Int.to_string n}]"])
    |> String.concat ~sep:""
  in
  ...
  let parent_action_nid_ex action =
    ...
    in
    let fallback () = [%string "A_%{string_of_action action}"] in
    match action with
    ...
    | _ -> fallback ()
  in
  (* For a summary (action, suffix), return a list of (node_id, tag, parent_nid) triples.
  ...
  let summary_item_triples action suffix =
    let canonical =
      (summary_nid action suffix, summary_label action suffix, parent_action_nid_ex action)
    in
    let probe_kind_of = function
      ...
      | _ -> None
    in
    match probe_kind_of action with
  ...
  add "graph LR";
  (* Artifact nodes — fetch action ID embedded in label when a Fetch action
  ...
  List.iter all_pool_kinds ~f:(fun kind ->
      if is_expanded kind then begin
        ...
        (* Identify the product variant (output of install/pack) based on which actions
           are present in the schema.  "staged" comes from Install_lib; "opam"/"pip"
           come from the relevant Publish action.  Everything else is an input variant. *)
        let product_vid =
          match kind with
          | Lib ->
              if List.exists actions ~f:(fun r -> Poly.equal r Install_lib)
        ...
        let label =
          if List.mem fetch_kinds kind ~equal:Poly.equal then begin
            let action_tag = string_of_action (Fetch kind) in
            let base = label_with_ids lbl action_tag in
  ...
  List.iter build_actions ~f:(fun r ->
      let name = string_of_action r in
      add [%string "    A_%{name}{{\"%{action_label name}\"}}"]);
  (* Install actions — hexagon shape *)
  List.iter install_actions ~f:(fun r ->
      let name = string_of_action r in
      add [%string "    A_%{name}{{\"%{action_label name}\"}}"]);
  ...
  List.iter publish_kinds ~f:(fun kind ->
      let name = string_of_action (Publish kind) in
  ...
  List.iter probe_kinds ~f:(fun kind ->
      if is_probe_expanded kind then
        ...
        Option.iter (probe_action_of_kind kind) ~f:(fun probe_action ->
          let name = string_of_action probe_action in
          let lbl = action_label_with_inspect name probe_action in
          add [%string "    A_%{name}([\"%{lbl}\"])"]));
  (* Summary actions — pill shape, one per (action, suffix) pair.
  ...
  List.iter summary_actions ~f:(fun (action, suffix) ->
      if not (is_inlined_inspect action) then begin
        let triples = summary_item_triples action suffix in
  ...
  List.iter publish_kinds ~f:(fun kind ->
      let tag = string_of_action (Publish kind) in
  ...
  List.iter probe_kinds ~f:(fun kind ->
      Option.iter (probe_action_of_kind kind) ~f:(fun probe_action ->
      let base_tag = string_of_action probe_action in
      if is_probe_expanded kind then
        ...
        (match kind with
         | Binding _ ->
             (* In the non-expanded overview, probe routes from the binding artifact
                pool node directly — the abstract action is "test the binding", whether
  ...
  List.iter summary_actions ~f:(fun (action, suffix) ->
      if not (is_inlined_inspect action) then
        List.iter (summary_item_triples action suffix) ~f:(fun (nid, tag, parent_nid) ->
  ...
  in
  let summary_entries =
    List.concat_map summary_actions ~f:(fun (action, suffix) ->
        if is_inlined_inspect action then []
        else List.map (summary_item_triples action suffix) ~f:(fun (nid, tag, _) -> (nid, tag)))
  in
  let action_entries =
    scan_entries
    @ (if has_configure then [ ("A_configure", "configure") ] else [])
    @ List.map build_actions ~f:(fun r ->
          ([%string "A_%{string_of_action r}"], string_of_action r))
    @ List.map install_actions ~f:(fun r ->
          ([%string "A_%{string_of_action r}"], string_of_action r))
    @ List.map publish_kinds ~f:(fun k ->
          let name = string_of_action (Publish k) in
          ([%string "A_%{name}"], name))
    @ List.concat_map probe_kinds ~f:(fun k ->
          if is_probe_expanded k then
            ...
            (match probe_action_of_kind k with
             | Some r ->
                 let name = string_of_action r in
                 [ ([%string "A_%{name}"], name) ]
             | None -> []))
...
(* ── Result diagram ──
   Reuses the action_graph schema diagram, colored by run status. *)

let result_status_of_run (steps : step list)
    (run_status : (string, step_status) Hashtbl.t) =
  let open Canary in
  let tbl = Hashtbl.create (module String) in
  (* Mark all store_actions actions as Not_in_spec initially.
  ...
  in
  List.iter (store_actions ~langs) ~f:(fun r ->
      Hashtbl.set tbl ~key:(string_of_action r) ~data:Not_in_spec);
  (* Override with actual run results.
     Also update the canonical action tag for split probes: probe_binding_pkg/pip
     have tag ≠ string_of_action, so merge them into "probe_binding" using
  ...
  List.iter steps ~f:(fun s ->
      ...
      let canonical = string_of_action s.action in
...
(* ── Step-based Mermaid renderer (per-view diagrams) ──
   Renders a (possibly filtered) step list. Each step becomes a
...
let _node_shape_of_action action =
  match action with
...
let mermaid_node_for_step ~id ~is_inspect ~is_scan (s : step) =
  let nid = "S_" ^ s.tag in
  let label = [%string "%{s.tag} [%{Int.to_string id}]"] in
  let shape =
    if is_inspect || is_scan then `Pill
    else _node_shape_of_action s.action
  in
...
let _is_inspect_step (s : step) =
  String.is_suffix s.tag ~suffix:"_inspect"
let _is_scan_step (s : step) =
...
let step_id_table (all_steps : step list) : (string, int) Hashtbl.t =
  let tbl = Hashtbl.create (module String) in
  List.iteri all_steps ~f:(fun i s ->
      Hashtbl.set tbl ~key:s.tag ~data:(i + 1));
  tbl

(* Render a filtered step list as a Mermaid graph. [all_steps]
...
let view_predicate (v : view) (s : step) : bool =
  match v with
  | `Source ->
...
(* Compute the expand_probe_kinds parameter for mermaid_of_action_graph_schema.
  ...
  if List.is_empty probe_steps then None
    ...
    let items = List.map probe_steps ~f:(fun s ->
        ...
        let id_part = match Hashtbl.find step_ids s.tag with
          ...
          | None -> ""
        in
        (* Use the concrete step tag (e.g. probe_lib_apt) not the action name (probe_lib),
...
let _fetch_pm_of_tag kind tag =
  let base = string_of_action (Fetch kind) in
...
let _art_variant_version ~variant_infos ~steps kind vid =
  ...
  match vid with
  | "build_tree" ->
      let s = List.find steps ~f:(fun s -> match kind, s.action with
  ...
  | "staged" ->
      let s = List.find steps ~f:(fun s ->
          match s.action with Install_lib -> true | _ -> false) in
      Option.bind s ~f:(fun s -> version_of_run_id s.variant_id)
  | pm ->
      let via_pack = List.find steps ~f:(fun s ->
          (match s.action with Publish k -> Poly.equal k kind | _ -> false)
          && String.equal (_fetch_pm_of_tag kind s.tag) pm) in
      (match via_pack with
       | Some s -> version_of_run_id s.variant_id
       | None ->
           let fetch_tag = string_of_action (Fetch kind) in
...
(* Compute the expand_artifact parameter for mermaid_of_action_graph_schema.
    ...
    let producer_id resolved_vid =
      let tag =
        if String.equal resolved_vid "build_tree" then Some build_tag
          ...
          | Some _ as s -> s
          | None ->
              let fetch_base = string_of_action (Fetch artifact_kind) in
              let cand =
...
let mermaid_full
  ...
  let binding_langs =
    List.filter_map steps ~f:(fun s -> match s.action with
  ...
  (* Build / install / pack / probe / summary steps.
     ...
     steps for fetch actions fall through here so they get a pill node. *)
  let is_fetch_or_follow s =
    not (String.is_suffix s.tag ~suffix:"_inspect")
    && ((match s.action with Fetch _ -> true | _ -> false)
        || String.equal s.tag "scan_source"
  ...
  List.iter kind_data ~f:(fun (kind, (variants, fsteps, build_tag, install)) ->
      if Poly.equal kind Source then ()  (* handled above *)
        ...
        Option.iter (probe_action_of_kind kind) ~f:(fun probe_action ->
        let probe_base = string_of_action probe_action in
        let probe_steps = List.filter steps ~f:(fun s ->
            not (String.is_suffix s.tag ~suffix:"_inspect")
...
let mermaid_view
    ...
    ?(step_ids : (string, int) Hashtbl.t option)
    ?(steps_by_action_tag : (string, string list) Hashtbl.t option)
    ?(summary_actions : (action * string) list option)
  ...
  let summary_tags_by_canonical =
    ...
    | Some sactions ->
        ...
        in
        let tbl = Hashtbl.create (module String) in
        List.iter sactions ~f:(fun (action, suffix) ->
            let canonical = string_of_action action ^ suffix in
            let action_base = string_of_action action in
            let concretes = List.filter_map steps ~f:(fun s ->
                if Poly.equal s.action action
                && String.is_suffix s.tag ~suffix:"_inspect" then
                  match String.chop_prefix s.tag ~prefix:action_base with
  ...
  | `Binding lang ->
      ...
      let expand =
        _compute_expand ~artifact_kind:(Binding lang)
          ...
          ~step_ids:sids steps
      in
      (match actions, step_ids, steps_by_action_tag, summary_actions with
       | Some actions, Some sids2, Some sbrt, Some sactions ->
           mermaid_of_action_graph_schema ?status ~has_scan:false
             ~summary_actions:sactions ~step_ids:sids2 ~steps_by_action_tag:sbrt
             ?summary_tags_by_canonical
             ?expand_artifact:expand ~expand_probe_kinds:all_probe_expand
             ~view_title:title ?focal_predicate:fp actions
       | _ ->
  ...
  | `Lib ->
      ...
      (match actions, step_ids, steps_by_action_tag, summary_actions with
       | Some actions, Some sids2, Some sbrt, Some sactions ->
           mermaid_of_action_graph_schema ?status ~has_scan
             ~summary_actions:sactions ~step_ids:sids2 ~steps_by_action_tag:sbrt
             ?summary_tags_by_canonical
             ?expand_artifact:expand ~expand_probe_kinds:all_probe_expand
             ~view_title:title ?focal_predicate:fp actions
  ...
  | _ ->
      ...
      (match actions, step_ids, steps_by_action_tag, summary_actions with
       | Some actions, Some sids2, Some sbrt, Some sactions ->
           mermaid_of_action_graph_schema ?status ~has_scan ~chain_scan
             ~summary_actions:sactions ~step_ids:sids2 ~steps_by_action_tag:sbrt
             ?summary_tags_by_canonical
...
let write_project_output ~dir ~project_name ~variant ~steps
  ...
  let summary_actions =
    let canonical_parent_tag action = string_of_action action in
    List.filter_map steps ~f:(fun s ->
        if String.is_suffix s.tag ~suffix:"_inspect" then
          let parent = canonical_parent_tag s.action in
  ...
  (* steps_by_action_tag: maps action name → concrete step tags for [N] label embedding.
  ...
  let mk_steps_by_action_tag ~include_scan =
    let tbl = Hashtbl.create (module String) in
    List.iter steps ~f:(fun s ->
        let skip =
          String.is_suffix s.tag ~suffix:"_inspect"
        ...
        in
        if not skip then
          let key = string_of_action s.action in
          Hashtbl.update tbl key ~f:(function
            | None -> [ s.tag ]
            | Some xs -> s.tag :: xs));
    tbl
  in
  let steps_by_action_tag_overview = mk_steps_by_action_tag ~include_scan:true in
  let steps_by_action_tag = mk_steps_by_action_tag ~include_scan:false in
  ...
  let overview_mmd =
    mermaid_of_action_graph_schema ~status:node_status ~has_scan:false
      ~summary_actions ~step_ids ~steps_by_action_tag:steps_by_action_tag_overview
      ~summary_tags_by_canonical
      (store_actions ~langs)
  in
  ...
  let full_mmd =
    mermaid_full ~status:node_status ~step_ids ~has_scan ~summary_actions
  ...
  List.iter views ~f:(fun v ->
      ...
      if not (List.is_empty filtered) then begin
        ...
        let sbrt = match v with
          | `Binding _ -> steps_by_action_tag_overview
          | _ -> steps_by_action_tag
        in
        let mmd = mermaid_view ~status:node_status ~view:v
            ~actions:(store_actions ~langs) ~step_ids ~steps_by_action_tag:sbrt
            ~summary_actions ~has_scan ~artifact_names ~variant_infos steps in
