...
(* ── Build-tool primitives (Step 4 (b), 2026-05-29) ─────────────────────
   Wrap cmake / ninja / dune / a trailing marker-write into named
   helpers, so project specs don't re-implement the same shell blocks.
   ...
   recognizes ("conf.ok", "build.ok", "probe.log", …). [with_marker]
   appends step (2); the [_cmd] primitives produce step (1). *)
