(** [project_run] — the interface a generic cross-project runner consumes
    ...
    - [pr_provenance] — STATIC, per-artifact provision detail for `spec` (no
      execution): the concrete backing the abstract [artifact_id] + [placement]
      can't carry — a vendored source PATH, or a PM + PACKAGE name, or the
      build-from-source origin. [None] = undeclared. A project DECLARES this;
      `spec` displays it so the artifact list is verifiably spec-sourced.
...
type project_run = {
  ...
  pr_provenance : Canary_enumerate.artifact_id -> string option;
}
